...
try
	...
	@printf("%s FAIL (please make sure that module is available to Julia) %s\n", KRED,KRES);
end

function TEST_parsePdb()
	@printf("Testing parsePdb ...")
	try
		pdb=pdbTool.parsePdb("5PTI.pdb");
		@printf("%s SUCCES%s\n",KGRN,KRES)
	catch
...
function TEST_mapChainToHmm()
	...
	try
		pdb=pdbTool.parsePdb("5PTI.pdb");
		pdbTool.mapChainToHmm(pdb.chain["A"],"Kunitz_BPTI.hmm")
		@printf("%s SUCCESS %s\n",KGRN,KRES)
	catch
		@printf("%s FAIL (function exited with error) %s\n", KRED, KRES);
		return false
...
function TEST_mapChainToHmmLegacy()
	...
	try
		pdb=pdbTool.parsePdb("5PTI.pdb");
		pdbTool.mapChainToHmmLegacy(pdb.chain["A"],"Kunitz_BPTI.hmm")
		@printf("%s SUCCESS\n%s",KGRN,KRES)
	catch
		@printf("%s FAIL (function exited with error) %s\n", KRED, KRES);
		return false
